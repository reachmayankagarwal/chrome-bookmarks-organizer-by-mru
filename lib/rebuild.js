...
async function rebuildFolder(folderId, options) {
  ...
  if (options.folderHashes[folderId] === desiredHash) {
    if (updatedCount > 0) {
      try {
        ...
        errors.push({ op: 'persist-duplicates', id: folderId, error: e });
      }
    }
    return { movedCount: 0, updatedCount, createdCount: 0, removedCount: 0, errors };
  }
  ...
  try {
    ...
    errors.push({ op: 'persist-folderHashes', id: folderId, error: e });
  }
